...
Claude Code / Cowork plugin that wraps the [`oni-vision`](..) SQLite output behind an MCP server, plus a skill that teaches the model how to use the tools. Drop this in your Claude Code or Cowork plugins directory and Claude can answer questions about your live ONI colony.
...
| `oni_freshness`  | Seconds since oni-vision last reparsed                            |
...
```bash
cd oni-vision-plugin
npm install
```

The oni-vision daemon (the parent project) needs to be running for there to be any data:
...
The exact mechanism depends on your client. Add this plugin's directory to your Claude Code or Cowork plugin path, then enable the `oni-vision` plugin in your client's plugin manager.
...
1. If `~/.oni-vision/config.json` (or `~/.config/oni-vision/config.json`) sets `outputDir`, it reads `<outputDir>/current.sqlite`.
2. Otherwise, falls back to `~/.oni-vision/output/current.sqlite` (oni-vision's default).

No plugin-specific config — it follows oni-vision's config so there's only one source of truth.
...
- Each call opens a fresh handle so we never serve stale rows after an atomic rename by oni-vision.
